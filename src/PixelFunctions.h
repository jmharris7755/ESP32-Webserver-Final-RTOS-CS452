...
void Rainbower::drawTest(){
  color1 = color2;
  stepVal1 = stepVal2;
  for (uint16_t i = 0; i < pStrand->numPixels; i++) {
    pStrand->pixels[i] = pixelFromRGBW(color1.r / color_div, color1.g / color_div, color1.b / color_div, color1.w /color_div);
    if (i == 1) {
      color2 = color1;
      stepVal2 = stepVal1;
    }
    switch (stepVal1) {
      case 0:
        color1.g += anim_step;
        color1.w += anim_step;
        if (color1.g >= anim_max && color1.w >= anim_max)
          stepVal1++;
        break;
      case 1:
        color1.g -= anim_step;
      ...
      case 2:
        color1.b += anim_step;
        color1.w += anim_step;
        if (color1.b >= anim_max && color1.w >= anim_max)
          stepVal1++;
        break;
      case 3:
        color1.b -= anim_step;
        color1.w -= anim_step;
        if (color1.b == 0 && color1.w == 0)
          stepVal1++;
        break;
      case 4:
        color1.r += anim_step;
        color1.w += anim_step;
        if (color1.r >= anim_max && color1.w >= anim_max)
          stepVal1++;
        break;
      case 5:
        color1.r -= anim_step;
        color1.w -= anim_step;
        if (color1.r == 0 && color1.w == 0)
          stepVal1 = 0;
        break;
    }
  }
...
void runledTest(strand_t * strands[], int numStrands, unsigned long delay_ms, unsigned long timeout_ms)
{
  //Rainbower rbow(pStrand); Rainbower * pRbow = &rbow;
  Rainbower * pRbow[numStrands];
  int i;
...
void ledTest(strand_t *pStrand, unsigned long delay_ms, unsigned long timeout_ms){

  strand_t *strands[] = { pStrand };
  runledTest(strands, 1, delay_ms, timeout_ms);
}
...
//function for displaying a single color to two pixels
void displayTwoPixels(int Pixel1, int Pixel2, pixelColor_t myStatusColor)
{

  strand_t * pStrand = &STRANDS[0];

  BC24clearStrip(pStrand);
  pStrand->pixels[Pixel1] = myStatusColor;
  pStrand->pixels[Pixel2] = myStatusColor;
  digitalLeds_updatePixels(pStrand);

}

//function for blinking a pixel witout clearning other pixels
void displaySinglePixelNC(int Pixel, pixelColor_t myStatusColor)
{
